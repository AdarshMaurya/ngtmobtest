package com.cognizant.ngtmobtest.ui.explorer;
...
public abstract class LazyLoadingTreeNode extends DefaultMutableTreeNode implements TreeWillExpandListener {
     ...
     */
    protected com.cognizant.ngtmobtest.ui.worker.SwingWorker<MutableTreeNode[], ?> createSwingWorker(final JTree tree) {

        com.cognizant.ngtmobtest.ui.worker.SwingWorker<MutableTreeNode[], ?> worker = new com.cognizant.ngtmobtest.ui.worker.SwingWorker<MutableTreeNode[], Object>() {
     ...
     */
    protected void registerSwingWorkerForCancel(JTree tree, com.cognizant.ngtmobtest.ui.worker.SwingWorker<MutableTreeNode[], ?> worker) {
        if (!cancelable) {
            return;
        }
    ...
    public void treeWillExpand(TreeExpansionEvent event) {
        if (this.equals(event.getPath().getLastPathComponent())) {
            ...
            setLoading();
            com.cognizant.ngtmobtest.ui.worker.SwingWorker<MutableTreeNode[], ?> worker = createSwingWorker(tree);
            worker.execute();
        }
    }
     ...
     * @param worker
     */
    protected void unRegisterSwingWorkerForCancel(JTree tree, com.cognizant.ngtmobtest.ui.worker.SwingWorker<MutableTreeNode[], ?> worker) {
        if (!cancelable) {
            return;
        }
        Action action = tree.getActionMap().get(ESCAPE_ACTION_NAME);
    ...
    protected static class CancelWorkersAction extends AbstractAction {
         ...
         * the SwingWorkers
         */
        private Vector<com.cognizant.ngtmobtest.ui.worker.SwingWorker<MutableTreeNode[], ?>> workers = new Vector<>();
        ...
        public void actionPerformed(ActionEvent e) {
            for (com.cognizant.ngtmobtest.ui.worker.SwingWorker<MutableTreeNode[], ?> worker : workers) {
                worker.cancel(true);
            }
         ...
         */
        public void addSwingWorker(com.cognizant.ngtmobtest.ui.worker.SwingWorker<MutableTreeNode[], ?> worker) {
            workers.add(worker);
        }
         ...
         */
        public void removeSwingWorker(com.cognizant.ngtmobtest.ui.worker.SwingWorker<MutableTreeNode[], ?> worker) {
